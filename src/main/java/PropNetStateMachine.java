...
public class PropNetStateMachine extends StateMachine {
    ...
    public int getGoal(MachineState state, Role role)
    	  ...
    	  for (int i=0; i<roles.size(); i++) {
    	      if (role==roles.get(i)) {
    	    	  rewards = propNet.getGoalPropositions().get(i);
    	    	  break;
    	    	  }
    	      }
    ...
    public boolean propmarkp(Component p) {
	    ...
	    if (propNet.getPropositions().contains(p)) {return propmarkp(p.getSingleInput());}
    ...
    public boolean markactions (List<Proposition> vector) {
    ...
    public MachineState getNextState(MachineState state, List<Move> moves)
    	...
    	markactions(ordering);
    	markbases(ordering);
    	Map<GdlSentence, Proposition> bases = propNet.getBasePropositions();
    	List<Proposition> nexts = new ArrayList<Proposition>();
    	for (int i=0; i<bases.size(); i++){
    	    nexts.get(i).setValue(propmarkp(bases.get(i).getSingleInput().getSingleInput()));
    	    }
    	return getStateFromBase();
    }
