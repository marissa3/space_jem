...
public class Space_JEM_comp2 extends StateMachineGamer {
	...
	public int delibMaxScore(Role role, MachineState state, StateMachine machine, long timeout, int level, int limit) throws MoveDefinitionException, GoalDefinitionException, TransitionDefinitionException
	{
		if (machine.isTerminal(state)){
			return machine.getGoal(state, role);
		}
		...
		for (Move m : moves){
			List<Move> ms = new ArrayList<Move>();
			ms.add(m);
			int result = delibMaxScore(role, machine.getNextState(state, ms), machine, timeout, level + 1, limit);
			if (result > score) score = result;
		}
		return  score;
	}
	public Move findBest1Player(Role role, MachineState state, long timeout) throws MoveDefinitionException, GoalDefinitionException, TransitionDefinitionException{
		StateMachine machine = getStateMachine();
		List<Move> moves = machine.getLegalMoves(state, role);
		System.out.println("print moves in findbest");
		for(Move m : moves){
		...
		for (Move m : moves){
			...
			int result = delibMaxScore(role, nextState, machine, timeout, level, limit);
			if (result == 100) return m;
			if (result > score){
				System.out.println(result);
				score = result;
	...
	public Move stateMachineSelectMove(long timeout)
		...
		if(machine.getRoles().size() == 1){
			return findBest1Player(role, state, timeout);
		}
